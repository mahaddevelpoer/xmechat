...
class _PrivateChatScreenState extends ConsumerState<PrivateChatScreen> {
  ...
  Future<void> _loadMessages() async {
    final msgs = await ref.read(chatServiceProvider).fetchMessages(widget.chatId);
    if (!mounted) return;
  ...
  Future<void> _loadOtherUser() async {
    ...
    final chat = (await ref.read(chatServiceProvider).fetchChats())
        .firstWhere((c) => c.id == widget.chatId, orElse: () => ChatModel(
          id: '', user1Id: '', user2Id: '', lastMessageAt: DateTime.now(), createdAt: DateTime.now()));
  ...
  Future<void> _sendImage({bool camera = false}) async {
    ...
    try {
      final msg = await ref.read(chatServiceProvider).sendMediaMessage(
        ...
        bytes: bytes, type: MessageType.image, fileName: img.name,
      );
  ...
  Future<void> _sendFile() async {
    ...
    setState(() => _uploading = true);
    try {
      final type = (file.extension?.toLowerCase() == 'mp3' ||
  ...
  Future<void> _sendLocation() async {
    try {
      ...
      final msg = await ref.read(chatServiceProvider).sendLocation(
        chatId: widget.chatId, receiverId: _otherUser?.id ?? '',
        lat: pos.latitude, lng: pos.longitude, locationName: 'My Location',
      );
      if (mounted) setState(() => _messages.add(msg));
      _scrollToBottom();
    } catch (e) {
  ...
  Future<void> _stopRecording() async {
    ...
    try {
      ...
      final msg = await ref.read(chatServiceProvider).sendMediaMessage(
        chatId: widget.chatId, receiverId: _otherUser?.id ?? '',
        bytes: audioBytes, type: MessageType.audio, fileName: 'voice_${DateTime.now().millisecondsSinceEpoch}.m4a',
      );
      if (mounted) setState(() => _messages.add(msg));
      _scrollToBottom();
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(streamMessagesProvider(widget.chatId), (_, next) {
      if (next.value != null) {
        final msgs = next.value!.map((m) => MessageModel.fromMap(m)).toList();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: GestureDetector(
          ...
          child: Row(children: [
            UserAvatar(
              url: _otherUser?.avatarUrl, name: _otherUser?.name ?? '?',
              isOnline: _otherUser?.isOnline ?? false, radius: 18),
            const SizedBox(width: 10),
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(_otherUser?.name ?? 'Loading...',
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
              Text(_otherUser?.isOnline == true ? 'online' : 'last seen recently',
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Wrap(spacing: 20, runSpacing: 16, children: [
          _AttachItem(icon: Icons.image, label: 'Gallery', color: Colors.purple, onTap: () { Navigator.pop(context); _sendImage(); }),
...
class _MessageActionsSheet extends StatelessWidget {
  ...
  const _MessageActionsSheet({
    required this.message, required this.myId,
    required this.onReply, required this.onCopy, required this.onStar,
        ...
        onTap: () {
          ...
          if (isMine) {
            showDialog(context: context, builder: (_) => AlertDialog(
              ...
              actions: [
                TextButton(onPressed: () { Navigator.pop(context); onDelete(false); }, child: const Text('Delete for Me')),
                TextButton(onPressed: () { Navigator.pop(context); onDelete(true); }, child: const Text('Delete for Everyone', style: TextStyle(color: AppColors.error))),
