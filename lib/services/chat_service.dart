...
class ChatService {
  ...
  Future<List<MessageModel>> fetchMessages(String chatId, {int limit = 50, int offset = 0}) async {
    final data = await _db.from(SupabaseConstants.messagesTable)
        .select('*, reactions(*)')
        .eq('chat_id', chatId)
    ...
    return data.map<MessageModel>((m) {
      ...
      if (m['reactions'] != null) {
        msg.reactions = (m['reactions'] as List).map((r) => ReactionModel.fromMap(r)).toList();
      }
      return msg;
    }).toList();
  }
  ...
  Future<MessageModel> sendTextMessage({
    required String chatId, required String receiverId, required String text,
    String? replyTo, String replyPreview = '', bool isForwarded = false,
  }) async {
    final data = await _db.from(SupabaseConstants.messagesTable).insert({
      'chat_id': chatId, 'sender_id': _uid, 'receiver_id': receiverId,
      'text': text, 'type': 'text', 'reply_to': replyTo,
      'reply_preview': replyPreview, 'is_forwarded': isForwarded, 'status': 'sent',
    }).select().single();
  ...
  Future<MessageModel> sendMediaMessage({
    ...
    final bucket = type == MessageType.audio ? SupabaseConstants.voiceNotesBucket
        : type == MessageType.document ? SupabaseConstants.documentsBucket
    ...
    await Supabase.instance.client.storage.from(bucket)
    ...
    final preview = type == MessageType.image ? '📷 Photo'
        : type == MessageType.audio ? '🎵 Voice note'
  ...
  Future<MessageModel> sendLocation({
    required String chatId, required String receiverId,
    required double lat, required double lng, String locationName = '',
  }) async {
    final data = await _db.from(SupabaseConstants.messagesTable).insert({
      'chat_id': chatId, 'sender_id': _uid, 'receiver_id': receiverId,
      'type': 'location', 'latitude': lat, 'longitude': lng,
      'location_name': locationName, 'status': 'sent',
    }).select().single();
  ...
  Future<MessageModel> sendContact({
    required String chatId, required String receiverId,
    required String contactName, required String contactPhone,
  }) async {
    final data = await _db.from(SupabaseConstants.messagesTable).insert({
      'chat_id': chatId, 'sender_id': _uid, 'receiver_id': receiverId,
      'type': 'contact', 'contact_name': contactName,
      'contact_phone': contactPhone, 'status': 'sent',
    }).select().single();
  ...
  Future<void> deleteMessage(String messageId, {required bool forEveryone}) async {
    if (forEveryone) {
      ...
      final msg = await _db.from(SupabaseConstants.messagesTable)
          .select('sender_id').eq('id', messageId).maybeSingle();
      if (msg == null) return;
      if (msg['sender_id'] == _uid) {
        await _db.from(SupabaseConstants.messagesTable)
            .update({'deleted_for_sender': true}).eq('id', messageId);
      } else {
        await _db.from(SupabaseConstants.messagesTable)
            .update({'deleted_for_receiver': true}).eq('id', messageId);
      }
    }
  }

  Future<void> addReaction(String messageId, String emoji) async {
    await _db.from(SupabaseConstants.reactionsTable)
        .upsert({'message_id': messageId, 'user_id': _uid, 'emoji': emoji});
  }
  ...
  Future<void> toggleStar(String messageId, bool star) async {
    if (star) {
      await _db.from(SupabaseConstants.starredMessagesTable)
          .insert({'user_id': _uid, 'message_id': messageId});
    } else {
      await _db.from(SupabaseConstants.starredMessagesTable)
          .delete().eq('user_id', _uid).eq('message_id', messageId);
    }
  }
  ...
  Future<void> blockUser(String targetId) async {
    await _db.from(SupabaseConstants.blockedUsersTable)
        .insert({'user_id': _uid, 'blocked_user_id': targetId});
  }

  Future<void> unblockUser(String targetId) async {
    await _db.from(SupabaseConstants.blockedUsersTable)
        .delete().eq('user_id', _uid).eq('blocked_user_id', targetId);
  }

  Future<bool> isUserBlocked(String targetId) async {
    final data = await _db.from(SupabaseConstants.blockedUsersTable).select('id')
        .eq('user_id', _uid).eq('blocked_user_id', targetId).maybeSingle();
    return data != null;
  }

  Future<void> _updateChatLastMessage(String chatId, String msg, String type) async {
    await _db.from(SupabaseConstants.chatsTable).update({
      'last_message': msg,
